...
import { getPayments, getRevenueSummary, createPayment, downloadInvoice } from '../../api/billing.api';
...
const EMPTY_FORM = {
  memberId: '', planId: '', method: 'cash',
  discount: '0', notes: '', transactionRef: '',
};

const STATUS_COLOR = {
  available:   { bg: 'rgba(26,106,74,0.12)',  color: '#1A6A4A', border: 'rgba(26,106,74,0.25)' },
  occupied:    { bg: 'rgba(201,168,76,0.18)', color: '#8a6d00', border: 'rgba(201,168,76,0.35)' },
  maintenance: { bg: 'rgba(0,0,0,0.06)',      color: '#7A7A6E', border: 'rgba(0,0,0,0.12)'     },
};
    ...
    <div>
      <div className="page-header">
        ...
        <div className="header-actions">
          <button className="btn btn-gold" onClick={openModal}>+ Record Payment</button>
        </div>
      </div>
      ...
      {summary && (
        <div className="stats-grid" style={{ marginBottom: 24 }}>
          <StatCard label="This Month"     value={`₹${(summary.monthlyRevenue || 0).toLocaleString('en-IN')}`} icon="📈" accent="gold" />
          <StatCard label="This Year"      value={`₹${(summary.yearlyRevenue  || 0).toLocaleString('en-IN')}`} icon="💰" accent="teal" />
          <StatCard label="Total Invoices" value={summary.totalPayments || 0}                                   icon="🧾" accent="success" />
          <StatCard
            label="Avg / Invoice"
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty-state"><div className="icon">🧾</div><div>No payments yet</div></div>
        ) : (
              ...
              <tbody>
                {payments.map(p => (
                  <tr key={p._id}>
                    ...
                    <td style={{ fontSize: '0.78rem', color: '#7A7A6E' }}>{p.planId?.name}</td>
                    <td style={{ fontWeight: 600 }}>₹{p.totalAmount?.toLocaleString('en-IN')}</td>
                    <td style={{ textTransform: 'capitalize', fontSize: '0.78rem' }}>{p.method}</td>
                    <td style={{ fontSize: '0.72rem', color: '#7A7A6E' }}>
                      {new Date(p.periodStart).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}
                      {' – '}
                      {new Date(p.periodEnd).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' })}
                    </td>
                    <td><Badge status={p.status} /></td>
                    <td>
                      <button className="btn btn-outline btn-sm" onClick={() => handleDownload(p._id, p.invoiceNo)}>
                        ⬇ PDF
                      </button>
                    </td>
                  </tr>
      ...
      <Modal
        ...
        title={step === 1 ? 'Record Payment' : '✅ Payment Recorded — Assign Seat'}
        ...
        {step === 1 && (
          <form onSubmit={handleCreatePayment}>
            <div className="form-group">
              <label>Member *</label>
              <select className="form-input" value={form.memberId} onChange={set('memberId')} required>
            ...
            <div className="form-group">
              <label>Plan *</label>
              <select className="form-input" value={form.planId} onChange={set('planId')} required>
                <option value="">Select plan…</option>
                {plans.map(p => (
                  <option key={p._id} value={p._id}>
                    {p.name} — ₹{p.price}/mo ({p.accessType})
                  </option>
                ))}
              </select>
            </div>
            <div className="grid-2">
              ...
              <div className="form-group">
                ...
                <input className="form-input" type="number" min="0" value={form.discount} onChange={set('discount')} />
              </div>
            </div>
            <div className="form-group">
              <label>Transaction Reference</label>
              <input className="form-input" value={form.transactionRef} onChange={set('transactionRef')} placeholder="UPI/card ref number" />
            </div>
            <div className="form-group">
              <label>Notes</label>
            ...
            {formErr && (
              <div style={{ color: '#C0392B', fontSize: '0.82rem', marginBottom: 12 }}>{formErr}</div>
            )}
          ...
          <div>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: '#7A7A6E', fontFamily: 'JetBrains Mono, monospace' }}>
                  {paidMember.memberId}
                </div>
              </div>
            </div>

            <div style={{ fontSize: '0.84rem', color: '#2C3E50', marginBottom: 14, fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              {['A', 'B'].map(z => (
                <button
                  ...
                  onClick={() => { setSeatZone(z); setSelectedSeat(null); }}
                >
                  Zone {z} ({availSeats.filter(s => s.zone === z).length} free)
                </button>
              ))}
            </div>
            ...
            {filteredSeats.length === 0 ? (
              <div style={{ color: '#7A7A6E', fontSize: '0.82rem', textAlign: 'center', padding: '20px 0' }}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background: isSelected ? '#0D0D0D'                   : STATUS_COLOR.available.bg,
                        color:      isSelected ? '#fff'                      : STATUS_COLOR.available.color,
                        border:     isSelected ? '2px solid #0D0D0D'         : `1px solid ${STATUS_COLOR.available.border}`,
            ...
            {selectedSeat && (
              <div style={{
                ...
                &nbsp;·&nbsp; Zone {selectedSeat.zone}, Floor {selectedSeat.floor}
                &nbsp;·&nbsp;
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              >
                {assigning ? 'Assigning…' : `Assign ${selectedSeat?.seatNumber || 'Seat'}`}
              </button>
            </div>
          </div>
        )}
